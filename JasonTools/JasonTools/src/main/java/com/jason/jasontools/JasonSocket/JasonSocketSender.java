    ...
    public synchronized void sendData(IProtocol protocol) {
        if (client == null) {
            if (resultListener != null) {
                resultListener.error("socket client 为空！");
            }
        ...
        if (client.getSocket() == null) {
            if (resultListener != null) {
                resultListener.error("socket 已关闭连接，请检查");
            }
        ...
        if (client.getSocket().isClosed() && !client.getSocket().isConnected()) {
            if (resultListener != null) {
                resultListener.error("socket 未建立连接，请检查");
            }
        ...
        try {
            ...
            client.getSocket().getOutputStream().write(protocol.getProtocol());
